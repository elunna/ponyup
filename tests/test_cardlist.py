...
def test_countrank_1K_returns1():
    cl = cardlist.CardList(tools.to_cards(['Kc']))
...
def test_countrank_1Kin2cards_returns1():
    cl = cardlist.CardList(tools.to_cards(['As', 'Kc']))
    assert cl.count_rank('K') == 1
...
def test_countsuit_1spade_returns1():
    cl = cardlist.CardList(tools.to_cards(['Kc', 'As']))
...
def test_peek_AsKs_stillhidden():
    cl = cardlist.CardList(tools.to_cards(['As', 'Ks']))
    cl.peek()
...
def test_remove_cardnotinlist_returnsNone():
    cl = cardlist.CardList(tools.to_cards(['Zs']))
...
def test_sort_2cards_deuceisfirst():
    cl = cardlist.CardList(tools.to_cards(['As', '2s']))
    cl.sort()
    assert cl.cards[0].rank == '2'


def test_sort_3cards_deuceisfirst():
    cl = cardlist.CardList(tools.to_cards(['As', '2s', '7s']))
    cl.sort()
    assert cl.cards[0].rank == '2'
...
def test_stripsuits_stripMultipleSuits_allSuitsWereStripped():
    cl = cardlist.CardList(tools.to_cards(['As', 'Kc', 'Qd']))
